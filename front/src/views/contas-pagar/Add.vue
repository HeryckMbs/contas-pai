    ...
    <v-card rounded="lg" style="height:100%" class="px-3 py-3">
        ...
        <v-card-text>
            <v-row>
                ...
                <v-col xs="12" sm="12" md="3" cols="3">
                    <v-select v-model="billToPay.status" item-title="name" prepend-icon="mdi-clock" label="Status"
                        :items="[
            ...
            <v-row>
                <v-col cols="12">
                    <v-select v-model="billToPay.recurrence" item-title="name" item-value="value"
...
export default {
    ...
    methods: {
        create() {
            ContasPagarService.create(this.billToPay).then((response) => {
                let title = ''
                let messages = [];
        ...
        getCategories() {
            ...
            CategoriaService.getAll('expense').then((response) => {
