<template>
    <v-card rounded="lg" style="height:100%" class="px-3 py-3">
        <template v-slot:title>
            <h4 class="font-weight-black">Edição da compra #{{ billToPay.id }}</h4>
        </template>
        ...
        <v-card-text>
            <v-row>
                ...
                <v-col cols="6">
                    <v-select item-title="title_name" v-model="billToPay.credit_card" prepend-icon="mdi-card"
                        label="Cartão de Crédito" :items="creditCards" item-value="id" variant="outlined"></v-select>
...
export default {
    ...
    methods: {
        create() {
            ShoppingService.update(this.billToPay).then((response) => {
                let title = ''
                let messages = [];
        ...
        findById(id: number) {
            ShoppingService.findById(id).then((response) => {
